...
RSpec.describe Gws::Schedule::PlanRepeat, type: :model, dbscope: :example do
  describe 'entry schedule' do
    it 'create daily plan' do
      create :gws_schedule_repeat, :daily

      expect(Gws::Schedule::Plan.count).to eq 5
    end

    it 'create weekly plan' do
      create :gws_schedule_repeat, :weekly
    ...
    describe 'create monthly plan' do
      it 'by date' do
        create :gws_schedule_repeat, :monthly_by_date

        expect(Gws::Schedule::Plan.count).to eq 6
      end

      it 'by week' do
        create :gws_schedule_repeat, :monthly_by_week
  ...
  describe 'delete schedule' do
    subject(:schedule_repeat) { create :gws_schedule_repeat, :daily }

    it 'delete all schedule' do
      schedule_repeat.destroy
      expect(Gws::Schedule::Plan.count).to eq 0
    end

    it 'delete schedule for april 3 and later' do
      schedule_repeat.delete_schedule_plan(Time.zone.local(2015, 4, 3))
      expect(Gws::Schedule::Plan.count).to eq 2
      expect(Gws::Schedule::Plan.order_by(:start_at.desc).first.start_at).to be < Time.zone.local(2015, 4, 3)
    end
  end

  describe 'get_date_next_specified_wday method' do
    subject(:sunday) { Time.zone.local(2015, 4, 5) }
    subject(:schedule_repeat) { build :gws_schedule_repeat }
  ...
  describe 'get_entry_dates_weekly method' do
    subject(:schedule_weekly) { build :gws_schedule_repeat, :weekly }
  ...
  describe 'get_entry_dates_monthly_by_date method' do
    subject(:schedule_monthly) { build :gws_schedule_repeat, :monthly_by_date }
  ...
  describe 'get_entry_dates_monthly_by_week method' do
    subject(:schedule_monthly) { build :gws_schedule_repeat, :monthly_by_week }
  ...
  describe 'get_date_by_ordinal_week method' do
    subject(:schedule_repeat) { build :gws_schedule_repeat, :monthly_by_week }
  ...
  describe 'entry_schedule_plan method' do
    ...
    it "create data check" do
      schedule_plan = Gws::Schedule::Plan.all.first
